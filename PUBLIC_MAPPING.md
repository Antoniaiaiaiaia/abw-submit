...
| `requirements` | **Main text block** (the "Senior Full Stack Engineer…" body) | **Routed to the Notion page body (正文) as paragraph blocks, NOT to a table property.** Notion rich_text properties cap around 2000 chars and render poorly for long listings; the admin's publishing workflow reads from the page body. The relay splits your text on double newlines (`\n\n`) into paragraphs for you. This is the single most important field — it's what readers actually read. Put the roles list + requirements here, in Chinese or bilingual. |
| `job_description` | Usually merged with `requirements` by admin, or omitted | If you only have one blob of text, put it all in `requirements`. Use `job_description` only if you clearly have two distinct short blocks (e.g. a one-line role summary vs. the full body). |
