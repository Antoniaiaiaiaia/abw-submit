...
| `apply_link` | **Link line** (`https://trustwallet.com/`) | **Must be the company's official website** (e.g. `trustwallet.com`, `lightcone.trade`). Not the aggregator URL where you found the listing. Usually discoverable from the page content — companies almost always link their own site. If you can't find it on the page, do a quick web search for the company name. **If the company has no discoverable official site, tell the user: "The admin won't approve submissions without a company official website — do you have one?"** before submitting. |
| `source_url` | **"更多在招" block at the bottom** | Shown at the bottom as `更多在招 / <url>`. Use for the external aggregator URL where you found the listing (ashbyhq, greenhouse, jobs.solana.com, lever, workable, etc.). This is the URL your agent fetched from — NOT the company site. |
